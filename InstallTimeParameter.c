/*
*  Tutorial: https://www.youtube.com/watch?v=wM19E3whV0c
*/
...
int64_t hook(uint32_t reserved ) {
    ...
    if(hook_param(SBUF(master_acc), "M", 1) != 20)
        NOPE("Misconfigured. Destination account not set as Hook Parameter.");
  /*
    uint8_t amount_param[8];
    if(hook_param(SBUF(amount_param), "AMT", 3) != 8)
        NOPE("Misconfigured. Amount not set as Hook Parameter.");

    uint8_t ledger[4];
    if(hook_param(SBUF(ledger), "LDGR", 4) != 4)
        NOPE("Misconfigured. Ledger Interval not set as Hook Parameter.");    

    TRACEHEX(master_acc);
    TRACEHEX(amount_param);
    TRACEHEX(ledger);
  */
